...
cj(function ($) {
    ...
    scheduler.config.collision_limit = 100; //allows creating 1 events per time slot
    ...
    $.validator.addMethod("startTimeClash", function (value, element) {
        ...
        return timeClash;
    }, ts("sStart time clashes with another slot item."));
    ...
    $(document).on("change", 'select[name="configuration"]', function (e) {
        var maxSize = $("#configSelect").find(':selected').data('maxsize');
       	//Aggiungere qui controllo maxSize
	 // var maxSize = 5;
	 if (maxSize) {
        	
	//	maxSize = "<?php $a=1; echo $a; ?>";	
	var xhttp = new XMLHttpRequest();	  
	xhttp.open("GET", "ajax_info.txt", true);
	xhttp.send();
	var booked = xhttp.responseText;

	//$('#max-quantity').text(ts('Posti Disponibili') + ': ' + maxSize + ts('Risorsa') + currentResource);
        $('#max-quantity').text(ts('Posti Disponibili') + ': ' + maxSize);
            ...
            $('#max-quantity').text('Nessun posto letto disponibile');
        }
        checkQuantityRestrictions();
    ...
    function checkQuantityRestrictions() {
        var maxSize = $("#configSelect").find(':selected').data('maxsize');
        //Aggiungere qui controllo maxSiz
	//var maxSize = 5;
