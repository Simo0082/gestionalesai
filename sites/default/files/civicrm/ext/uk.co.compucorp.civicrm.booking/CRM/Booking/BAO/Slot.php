 ...
 */
  static function getSlotBetweenDate($from, $to){
	...
	//RICERCO TUTTI I RESOURCE ID PER L'UTENTE AUTENTICATO
	...
	$conn = new mysqli($servername, $username, $password,$dbname);
        ...
        //SE L'UTENTE E' AMMINISTRATORE O UTENTE CON DIRITTI DI GESTIRE TUTTE LE STRUTTURE
        if (array_search('administrator',$user->roles) > 0 || array_search('casc',$user->roles) > 0)
                $sql = "SELECT id FROM civicrm_booking_resource";
        else	
		$sql = "SELECT id FROM civicrm_booking_resource where owner = '".$user_role."'";
	...
	while($row = $result->fetch_assoc()) {
		array_push($array_result,$row["id"]);
	}
	#echo var_dump($array_result);
	    ...
	    //if(!array_search($tempArray, $orderResource)){	
            //MODIFICA QUI PER FILTRO RESOURCE
	if (array_search('administrator',$user->roles) > 0 || array_search('casc',$user->roles) > 0){
		if(!array_search($tempArray, $orderResource)){
	    ...
	    if(!array_search($tempArray, $orderResource) && in_array($tempArray['id'],$array_result,true)){
                $orderResource[$key] = $tempArray;
            }
           //echo var_dump($orderResource);
	}
