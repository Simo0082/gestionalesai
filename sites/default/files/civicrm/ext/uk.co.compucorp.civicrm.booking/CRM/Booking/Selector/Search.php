...
class CRM_Booking_Selector_Search extends CRM_Core_Selector_Base implements CRM_Core_Selector_API {
  ...
  function &getRows($action, $offset, $rowCount, $sort, $output = NULL) {
   ...
   //lets handle view, edit and delete separately.
	...
	//SE L'UTENTE E' DI TIPO STRUTTURA PRELEVO TUTTE LE PRENOTAZIONI RELATIVE ALL'UTENTE AUTENTICATO
	
	if (strpos($user_role, 'receptionist') !== false){
	//echo "SEI AUTENTICATO COME UTENTE DI STRUTTURA<br>";	
	...
	//echo "<br>ARRAY RISORSE DELL'UTENTE".var_dump($array_result);


	//ELIMINAZIONE BOOKING NON DI APPARTENENZA
	
	foreach ($rows as $booking_entry => $id) {
		if(!in_array($id['booking_id'],$array_result,true))
			unset($rows[$booking_entry]);
			
	}	
	}//FINE FILTRO PER UTENTI DI TIPO STRUTTURA
	return $rows;
  }
  ...
  public function &getColumnHeaders($action = NULL, $output = NULL) {
    if (!isset(self::$_columnHeaders)) {
      self::$_columnHeaders = array(
        ...
        // ),
        array(
          'name' => E::ts('Richiedente'),
          'sort' => 'booking_associated_contact',
          'direction' => CRM_Utils_Sort::DONTCARE,
        ),
        array(
          'name' => E::ts('Data Prenotazione'),
